...
export class UpdateSubRegionField_Transaction extends jsTPS_Transaction {
    constructor(regionID, field, value, prev, callback) {
        super();
    ...
    async doTransaction() {
		const { data } = await this.updateFunction({ variables: {regionId: this.regionID, field: this.field, value: this.value}});
		return data;
    }
    async undoTransaction() {
		const { data } = await this.updateFunction({ variables: {regionId: this.regionID, field: this.field, value: this.prev}});
		return data;
    }
}
